...
form.addEventListener('submit', (event) => {
    ...
    const animalId = form.dataset.id;
    ...
    fetch(`/api/animais/atualizar/${animalId}`, { 
        method: 'PUT', 
        headers: {
    ...
    .then(response => {
        if (response.ok) {
            ...
            response.json().then(errData => {
                console.error('Erro ao atualizar o animal: ' + (errData.message || 'Verifique os dados.'));
            }).catch(() => {
                console.error('Erro ao atualizar o animal. Verifique os dados.');
            });
        }
    })
    .catch(error => {
...
document.addEventListener('DOMContentLoaded', verificarAreas);